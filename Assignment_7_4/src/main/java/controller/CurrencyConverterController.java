...
public class CurrencyConverterController {
    ...
    private void initializeApplication() {
        try {
            ...
            // Initialize currencies if database is empty (first run)
            initializeCurrenciesIfEmpty();
            ...
            System.out.println("JPA Currency Converter with Transactions initialized successfully with " + currencies.size() + " currencies");
    ...
    private void initializeCurrenciesIfEmpty() {
        try {
            List<Currency> existingCurrencies = currencyDao.findAll();
            ...
            if (existingCurrencies.isEmpty()) {
                ...
                Currency[] defaultCurrencies = {
                        new Currency("USD", "US Dollar", 1.000000),
                        new Currency("EUR", "Euro", 0.920000),
                        new Currency("GBP", "British Pound", 0.790000),
                        new Currency("JPY", "Japanese Yen", 149.500000),
                        new Currency("CAD", "Canadian Dollar", 1.370000),
                        new Currency("AUD", "Australian Dollar", 1.540000),
                        new Currency("CHF", "Swiss Franc", 0.880000),
                        new Currency("CNY", "Chinese Yuan", 7.240000),
                        new Currency("SEK", "Swedish Krona", 10.870000),
                        new Currency("NZD", "New Zealand Dollar", 1.680000)
                };

                // Persist default currencies
                for (Currency currency : defaultCurrencies) {
                    currencyDao.persist(currency);
                }

                System.out.println("Successfully initialized " + defaultCurrencies.length + " default currencies");
            }
        } catch (Exception e) {
            System.err.println("Failed to initialize default currencies: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void handleConversion() {
        ...
        try {
            ...
            if (fromCurrency.equals(toCurrency)) {
                view.showError("Please select different source and target currencies.");
                return;
            }
            ...
            // Calculate the effective exchange rate for this transaction
            double effectiveRate = result / amount;

            // Display result
            view.getResultField().setText(String.format("%.2f", result));

            // Create and store transaction
            Transaction transaction = new Transaction(fromCurrency, toCurrency, amount, result, effectiveRate);
            boolean transactionStored = transactionDao.persist(transaction);

            if (transactionStored) {
                view.setStatus("Conversion completed and transaction stored in database");
                System.out.println("Transaction stored: " + amount + " " + fromCurrency.getAbbreviation() +
                        " -> " + result + " " + toCurrency.getAbbreviation());
            } else {
                view.setStatus("Conversion completed but failed to store transaction");
            ...
            handleDatabaseError("Failed to process conversion", e);
        }
    }
    ...
    private void handleDatabaseConnectionError() {
        ...
        alert.setContentText("Unable to establish connection to the MariaDB database.\n\n" +
                ...
                "2. Database 'currency_converter2' exists\n" +
